#%% Different Hamiltonian work extraction
...
for k in [0.01,0.4]:
    for f in [-1,1]:
        ...
        eb = 2/beta
        x=0
        gx_beta_vals = np.linspace(-0.5,0.5,1000)
        gx_range = (1/beta)*np.ones(len(gx_beta_vals))*gx_beta_vals
        for gx in gx_range:
...
#%% Efficiency against kappa for set g vals - x ham
...
efficiency_vals = np.zeros([100,5])
...
plt.figure()
plt.plot(kappa_range,efficiency_vals[:,0],label='$g_z=-0.2kT$')
plt.plot(kappa_range,efficiency_vals[:,1],label='$g_z = -0.1kT$')
plt.plot(kappa_range,efficiency_vals[:,2],label='$g_z = 0kT$')
plt.plot(kappa_range,efficiency_vals[:,3],label='$g_z = 0.1kT$')
plt.plot(kappa_range,efficiency_vals[:,4],label='$g_z = 0.2kT$')
